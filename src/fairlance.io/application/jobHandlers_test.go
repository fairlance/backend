...
import (
	...
	"fairlance.io/application/mocks"
)

func TestJobIndexJob(t *testing.T) {
	jobRepositoryMock := mocks.JobRepository{}
	...
	var jobContext = &application.ApplicationContext{
	...
	application.IndexJob(w, r)

	is.Equal(w.Code, http.StatusOK)
	var body []application.Job
...
func TestJobAddJob(t *testing.T) {
	...
	application.AddJob(&application.Job{
		Name:     "Name1",
		Summary:  "Summary1",
		Details:  "Details1",
		ClientID: 1,
...
func TestJobGetJobByIDReceivesTheRightID(t *testing.T) {
	...
	application.GetJobByID(uint(1)).ServeHTTP(w, r)
...
func TestJobGetJobByID(t *testing.T) {
	jobRepositoryMock := mocks.JobRepository{}
	jobRepositoryMock.GetJobCall.Returns.Job = application.Job{
		Model: application.Model{
			ID: 123456789,
		},
		Name:     "Name1",
	...
	application.GetJobByID(uint(0)).ServeHTTP(w, r)
...
func TestJobAddJobError(t *testing.T) {
	...
	application.AddJob(&application.Job{}).ServeHTTP(w, r)
...
func TestJobHandleApplyForJob(t *testing.T) {
	...
	application.ApplyForJob{jobApplication}.ServeHTTP(w, r)
...
func TestJobHandleApplyForJobError(t *testing.T) {
	jobRepositoryMock := mocks.JobRepository{}
	...
	application.ApplyForJob{&application.JobApplication{}}.ServeHTTP(w, r)
...
func TestJobWithJob(t *testing.T) {
	...
	next := func(job *application.Job) http.Handler {
	...
	application.WithJob{next}.ServeHTTP(w, r)
...
func TestJobHandleApplyingForJob(t *testing.T) {
	var jobContext = &application.ApplicationContext{}
	is := is.New(t)
	w := httptest.NewRecorder()
	requestBody := `{
	...
	next := func(jobApplication *application.JobApplication) http.Handler {
		return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
	...
	application.WithJobApplication{uint(1), next}.ServeHTTP(w, r)

	is.Equal(w.Code, http.StatusOK)
}

func TestJobHandleApplyingForJobError(t *testing.T) {
	var jobContext = &application.ApplicationContext{}
	is := is.New(t)
	w := httptest.NewRecorder()
	requestBody := `{}`
...
func TestJobHandleApplyingForJobErrorBadJSON(t *testing.T) {
	var jobContext = &application.ApplicationContext{}
	is := is.New(t)
	w := httptest.NewRecorder()
	requestBody := `{bad:json}`
...
func getRequest(jobContext *application.ApplicationContext, requestBody string) *http.Request {
