...
const (
	...
	insertTransactionSQL = `INSERT INTO transactions (track_id, provider, provider_key, project_id, amount, status, created_at, updated_at) VALUES ($1,$2,$3,$4,$5,$6,$7,$8) RETURNING id`
	selectTransactionSQL = `SELECT id, track_id, provider, provider_key, project_id, amount, status, created_at, updated_at FROM transactions WHERE track_id = $1`
	selectReceiverSQL    = `SELECT id, fairlance_id, email, amount FROM receivers WHERE transaction_id = $1`
	updateTransactionSQL = `UPDATE transactions SET status=$1, updated_at=$2 WHERE track_id=$3`
)
...
type db interface {
	...
	updateStatus(t transaction) error
}
...
func (db *sqlDB) updateStatus(t transaction) error {
	_, err := db.storage.Exec(updateTransactionSQL,
		t.status, time.Now(), t.trackID,
	)
	return err
}
